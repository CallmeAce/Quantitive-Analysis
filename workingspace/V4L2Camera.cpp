...
V4L2Camera::init(unsigned int width, unsigned int height, int numBuffers)
{
  ...
  m_Format.fmt.pix.pixelformat = V4L2_PIX_FMT_RGB24;
  m_Format.fmt.pix.field       = V4L2_FIELD_INTERLACED;
  if (!sendIoctlRequest(VIDIOC_S_FMT, &m_Format)) return false;

  if (m_Format.fmt.pix.pixelformat != V4L2_PIX_FMT_RGB24) {
    std::cerr << "V4L2Camera::init v4l2 did not accept RGB24 format.\n";
    return false;
  }
...
V4L2Camera::disconnect()
{
  ...
  for (int i = 0; i < m_NumDataBuffers; ++i)
    v4l2_munmap(m_DataBuffers[i].data, m_DataBuffers[i].length);
